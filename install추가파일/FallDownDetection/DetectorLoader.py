...
class TinyYOLOv3_onecls(object):
    ...
    def __init__(self,
        ...
        inx = os.path.realpath(__file__).find('DetectorLoader.py')
        self.model = Darknet(os.path.realpath(__file__)[:inx] + config_file).to(device)
        self.model.load_state_dict(torch.load(os.path.realpath(__file__)[:inx] + weight_file))
        self.model.eval()
        self.device = device
