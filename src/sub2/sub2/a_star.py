...
class a_star(Node):
    ...
    def __init__(self):
        ...
        self.goal_sub = self.create_subscription(PoseStamped,'next_pose',self.goal_callback,1)
    ...
    def dijkstra(self,start):
        ...
        while Q.qsize() != 0:
            current = Q.get()
            if self.cost[self.goal[0]][self.goal[1]] < current[0]:
                break

            for i in range(8):
                next = [current[0] + self.dCost[i], current[1]+self.dx[i], current[2]+self.dy[i]]
                if next[1] >= 0 and next[2] >= 0 and next[1] < self.GRIDSIZE and next[2] < self.GRIDSIZE:
                        if self.grid[next[1]][next[2]] < 50:
                            if next[0] < self.cost[next[1]][next[2]] :
                                Q.put(next)
                                self.path[next[1]][next[2]] = [current[1], current[2]]
                                self.cost[next[1]][next[2]] = current[0]
                            if next[1] == self.goal[0] and next[2] == self.goal[1] :
                                found = True
